...
#define AGB_ROM ((unsigned char *)GBA_ROM_BASE_ADDRESS)
#define AGB_SRAM ((volatile unsigned char *)GBA_SAVE_BASE_ADDRESS)
#define SRAM_SIZE 64
#define AGB_SRAM_SIZE (SRAM_SIZE * 1024)
#define AGB_SRAM_WINDOWED(i) (AGB_SRAM + ((i) & (AGB_SRAM_SIZE - 1)))
#define SRAM_BANK_SEL8 (*(volatile unsigned char *)GBA_SRAM_BANK_SELECT_ADDRESS)
#define SRAM_BANK_SEL16 (*(volatile unsigned short *)GBA_SRAM_BANK_SELECT_ADDRESS)
...
run_from_ram:
    ...
    mov r8, sp
    subs r11, r7, r6
    beq run_from_ram_invalid
    tst r11, # 3
    bne run_from_ram_invalid
    cmp r11, # 0x100
...
run_from_ram_revalidate_workspace:
    # Revalidate immediately before the first write to the candidate.
    bl run_from_ram_candidate_valid
    cmp r0, # 0
    beq run_from_ram_next_workspace

    # 0x100..0x11f: dispatcher metadata.
    add r0, r9, # 0x100
    ldr r1, =0x4b535742
    str r1, [r0, # 0]
    str r10, [r0, # 4]
    ...
    # 0x120..0x13f: stack-overflow guard.
    add r0, r9, # 0x120
    mov r2, # 0
run_from_ram_write_guard:
    eor r3, r1, r2
    str r3, [r0], # 4
    add r2, r2, # 1
    cmp r2, # 8
    blo run_from_ram_write_guard

    # 0x000..0x0ff: copied driver.
    mov r0, r6
    mov r1, r9
run_from_ram_copy_workspace:
    ldr r2, [r0], # 4
    str r2, [r1], # 4
    cmp r0, r7
...
run_from_ram_restore_workspace:
    str r10, [r0], # 4
    cmp r0, r1
    blo run_from_ram_restore_workspace
    mov r0, r9
run_from_ram_verify_restore:
    ldr r2, [r0], # 4
    cmp r2, r10
    movne r11, # 0
    cmp r0, r1
    blo run_from_ram_verify_restore
...
run_from_ram_candidate_dma:
    ldr r0, =0x040000b0
    mov r1, # 4
run_from_ram_candidate_dma_loop:
    ldr r2, [r0, # 0]
    cmp r2, r9
    blo run_from_ram_candidate_dma_destination
    cmp r2, r12
    blo run_from_ram_candidate_bad
run_from_ram_candidate_dma_destination:
    ldr r2, [r0, # 4]
    cmp r2, r9
    blo run_from_ram_candidate_dma_next
    cmp r2, r12
    blo run_from_ram_candidate_bad
run_from_ram_candidate_dma_next:
    add r0, r0, # 12
    subs r1, r1, # 1
    bne run_from_ram_candidate_dma_loop

    ldr r10, [r9]
    cmp r10, # 0
    beq run_from_ram_candidate_scan
    mvn r0, # 0
    cmp r10, r0
    bne run_from_ram_candidate_bad
run_from_ram_candidate_scan:
    mov r0, r9
run_from_ram_candidate_scan_loop:
    ldr r1, [r0], # 4
    cmp r1, r10
    bne run_from_ram_candidate_bad
    cmp r0, r12
    blo run_from_ram_candidate_scan_loop
    mov r0, # 1
    bx lr

run_from_ram_candidate_bad:
    mov r0, # 0
    bx lr
